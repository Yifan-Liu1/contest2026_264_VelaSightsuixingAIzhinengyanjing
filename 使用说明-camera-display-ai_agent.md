> **2026-09 更新：第三节「Camera」的第 10.10/10.13 两节已过时，帧率数字与本节主体（480x480/640x480 硬件 JPEG 状态）也已过时。**
> 10.10/10.13 记录的是 2026-08-17 那次改动把 `/dev/video0` 收口为纯软件编码器之后的状态
> （`CONFIG_BK7258_CAMERA_JPEG_SW` 已删除、640x480 n=10 只测出 1.26 fps、480x480 多帧
> 采集 `FAILED (-110)`）。此后两次改动已经推翻这些结论：
>
> - `07250ad fix(camera): stop the 480x480 capture after three frames, and un-throttle it`
>   ——修好了 480x480 多帧采集停在第 3 帧的问题（消费者线程优先级低于 LPWORK 被饿死），
>   同时去掉了拖慢 640x480 一倍多的定速器。
> - `4ca2746 perf(bk7258): take the hardware JPEG path from half the sensor rate to all
>   of it`——硬件 JPEG 通路已恢复且大幅提速：**640x480 实测 24.36–29.18 fps，480x480
>   实测 17.39–17.87 fps**（`hw_busy=0` 时），与本节 10.10/10.13 记录的软件编码器数字
>   （约 1.26–3.7 fps）不是同一回事。
>
> **当前权威的性能基线、判据与回归命令见 `docs/上板回归清单.md`**（2026-09-03 基线），
> 不要再以本文件第十节的数字作为判断依据。第一、二、四至九节（双屏分工、识别性能、
> Display、AI Agent/MiMo 接入、配置持久化）未受这两次改动影响，仍可参考。
>
> ---
>
> **2026-08-18 最终策略更新：** 本文中“仅软件 JPEG”“480x480 多帧失败”以及“硬件 JPEG 普遍不可用”的段落是修复前的历史测量。当前固件在 480x480/640x480 使用完整熵校验、fail-closed 的硬件 JPEG；864x480 因解码像素不稳定自动回退软件。
> 原文这里引用的 `docs/2026-08-18-三项修复实测记录.md` 在仓库历史中从未存在（核对 `git log
> --all --diff-filter=A` 无结果），已按当前实况改为指向 `docs/上板回归清单.md`（含
> 2026-09-03 基线）与第十节 10.10（硬件 JPEG 收口记录）——这两份才是能查到的最终证据。
